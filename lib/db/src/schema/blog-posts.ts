...
export const blogPostsTable = pgTable(
  "blog_posts",
  {
    ...
    primaryAuthorUrl: text("primary_author_url").notNull().default("/sa/about"),
    legalReviewerName: text("legal_reviewer_name").notNull().default("Omar Al-Baghdadi"),
    legalReviewerNameAr: text("legal_reviewer_name_ar").notNull().default("عمر البغدادي"),
    legalReviewerUrl: text("legal_reviewer_url").notNull().default("/sa/about"),
    ...
    correctionUrl: text("correction_url").notNull().default("/sa/contact?subject=article-correction"),
    published: boolean("published").notNull().default(false),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  },
  (table) => [
    index("blog_posts_published_date_idx").on(table.published, table.date),
  ],
);
