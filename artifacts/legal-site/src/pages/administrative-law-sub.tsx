...
export default function AdministrativeLawSub() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
        <div className="text-center">
          ...
          <Link href={`${regionPrefix}/services/administrative-law`} className="text-primary hover:underline">{rel.notFoundLink}</Link>
        </div>
      </div>
    );
  }
  ...
  const SEO_DATA: Record<string, { desc: string; descAr: string; kw: string; kwAr: string }> = {
    "board-of-grievances": {
      desc: "Board of Grievances legal advice in Saudi Arabia — filing administrative claims, hearings & appealing government decisions. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "administrative-appeals": {
      desc: "Administrative decision appeal legal advice in Saudi Arabia — challenging government & regulatory decisions. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "government-contracts": {
      desc: "Government contract dispute legal advice in Saudi Arabia — payment delays, performance disputes & Board of Grievances claims. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "public-procurement": {
      desc: "Public procurement & government tender legal advice in Saudi Arabia — bidding disputes, exclusion challenges & contract awards. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "regulatory-licensing": {
      desc: "Regulatory licensing dispute legal advice in Saudi Arabia — license refusals, suspensions & appeals before regulatory bodies. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "customs-tax-disputes": {
      desc: "Customs & tax dispute legal advice in Saudi Arabia — ZATCA claims, import duty disputes & administrative tax appeals. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "disciplinary-proceedings": {
      desc: "Professional disciplinary & government employment legal advice in Saudi Arabia — suspension defence, dismissal & appeals. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "government-compensation": {
      desc: "Government compensation claim legal advice in Saudi Arabia — expropriation, public works harm & administrative damages. Professional response within 24 hours via WhatsApp or email — CounselO.",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: isRTL ? "القانون الإداري" : "Administrative Law", item: "https://counselo.com/services/administrative-law" },
      { "@type": "ListItem", position: 4, name: data.title, item: `https://counselo.com/services/administrative-law/${subId}` },
    ],
  };
  ...
  const legalServiceSchema = {
    ...
    "url": `https://counselo.com/services/administrative-law/${subId}`,
      ...
      <div className="bg-card border-b border-border py-4 mt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center gap-1 text-sm text-muted-foreground">
            ...
            <ChevronRight className={`h-3.5 w-3.5 shrink-0 ${isRTL ? "rotate-180" : ""}`} />
            <Link href={`${regionPrefix}/services/administrative-law`} className="hover:text-primary transition-colors">{rel.breadcrumb.parent}</Link>
            <ChevronRight className={`h-3.5 w-3.5 shrink-0 ${isRTL ? "rotate-180" : ""}`} />
      ...
      <section className="relative py-16 overflow-hidden"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Link href={`${regionPrefix}/services/administrative-law`}
            ...
            <motion.div initial={{ opacity: 0, x: isRTL ? -20 : 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              ...
              <div className="bg-card border border-border p-6">
                ...
                <div className="space-y-1">
                  {otherAreas.map((area) => (
                    <Link key={area.id} href={`${regionPrefix}/services/administrative-law/${area.id}`}
