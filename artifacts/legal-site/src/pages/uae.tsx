      ...
      schema={[
        {
          ...
          "@type": "Service",
          "@id": `https://counselo-legal.com/#uae-service-${service.slug}`,
  ...
  return (
    <article className="uae-site uae-detail">
      ...
      <div className="uae-shell uae-detail-grid">
        <main><section><h2>{lang === "ar" ? "كيف نساعد" : "How we help"}</h2><p>{service.overview[lang]}</p></section><section><h2>{lang === "ar" ? "ما تغطيه الخدمة" : "What this service covers"}</h2><ul className="uae-check-list">{service.covers[lang].map((item) => <li key={item}><Check />{item}</li>)}</ul></section><section><h2>{lang === "ar" ? "مفاهيم قانونية رئيسية" : "Key UAE legal concepts"}</h2><ul className="uae-concepts">{service.concepts[lang].map((item) => <li key={item}>{item}</li>)}</ul></section><section><h2>{lang === "ar" ? "خدمات مرتبطة" : "Related legal services"}</h2><div className="uae-related-links">{related.map((item) => <Link key={item.slug} href={`${regionPrefix}/services/${item.slug}`}>{item.title[lang]}<Arrow rtl={isRTL} /></Link>)}<Link href={lang === "ar" ? "/blog/ar" : "/blog"}>{lang === "ar" ? "مركز كاونسلو للمعرفة القانونية" : "CounselO legal knowledge centre"}<Arrow rtl={isRTL} /></Link></div></section></main>
