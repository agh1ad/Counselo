...
export default function EnforcementSub() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
        <div className="text-center">
          ...
          <Link href={`${regionPrefix}/services/enforcement`} className="text-primary hover:underline">{rel.notFoundLink}</Link>
        </div>
      </div>
    );
  }
  ...
  const SEO_DATA: Record<string, { desc: string; descAr: string; kw: string; kwAr: string }> = {
    "judgment-enforcement": {
      desc: "Court judgment enforcement lawyer in Saudi Arabia — Saudi and foreign judgment enforcement, bank garnishment & asset attachment. Professional response within 24 hours via WhatsApp or email — CounselO Saudi Arabia.",
    ...
    "debt-recovery": {
      desc: "Commercial debt recovery lawyer in Saudi Arabia — recovering unpaid debts, dishonoured cheques, contract debts & invoices. Professional response within 24 hours via WhatsApp or email — CounselO Saudi Arabia.",
    ...
    "asset-attachment": {
      desc: "Asset attachment & freezing order lawyer in Saudi Arabia — precautionary bank account attachment, real estate freeze & property seizure. Professional response within 24 hours via WhatsApp or email — CounselO.",
    ...
    "travel-bans": {
      desc: "Travel ban lawyer in Saudi Arabia — applying for & defending against exit restriction and travel ban orders. Professional response within 24 hours via WhatsApp or email — CounselO Saudi Arabia.",
    ...
    "enforcement-against-government": {
      desc: "Enforcement against Saudi government entities lawyer — enforcing judgments against ministries, authorities & state enterprises. Professional response within 24 hours via WhatsApp or email — CounselO.",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: isRTL ? "التنفيذ وتحصيل الديون" : "Enforcement & Debt Collection", item: "https://counselo.com/services/enforcement" },
      { "@type": "ListItem", position: 4, name: data.title, item: `https://counselo.com/services/enforcement/${subId}` },
    ],
  };
  ...
  const legalServiceSchema = {
    ...
    url: `https://counselo.com/services/enforcement/${subId}`,
      ...
      <div className="bg-card border-b border-border py-4 mt-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center gap-1 text-sm text-muted-foreground">
            ...
            <ChevronRight className={`h-3.5 w-3.5 shrink-0 ${isRTL ? "rotate-180" : ""}`} />
            <Link href={`${regionPrefix}/services/enforcement`} className="hover:text-primary transition-colors">{rel.breadcrumb.parent}</Link>
            <ChevronRight className={`h-3.5 w-3.5 shrink-0 ${isRTL ? "rotate-180" : ""}`} />
      ...
      <section className="relative py-16 overflow-hidden"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Link href={`${regionPrefix}/services/enforcement`}
            ...
            <motion.div initial={{ opacity: 0, x: isRTL ? -20 : 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              ...
              <div className="bg-card border border-border p-6">
                ...
                <div className="space-y-1">
                  {otherAreas.map((area) => (
                    <Link key={area.id} href={`${regionPrefix}/services/enforcement/${area.id}`}
